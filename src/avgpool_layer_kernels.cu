...
__global__ void forward_avgpool_layer_kernel(int n, int w, int h, int c, float *input, float *output)
{
    ...
    int k = id % c;  //calculate which channel is beeing processed now
    id /= c;         //calculate which image is beeing process now
    int b = id;
    ...
    for(i = 0; i < w*h; ++i){
        ...
        output[out_index] += input[in_index];  //all of pixels in an image(in_index) is mapped to a single pixel in output(out_index)
    }
...
{
    size_t n = layer.c*layer.batch;  //n is channel*batch
    //every thread calculate a single channel(total number is n)
