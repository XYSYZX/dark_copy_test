...
{
    char labelpath[4096];
    find_replace(path, "images", "labels", labelpath);  //替换/images/COCO..为/labels/COCO..
    find_replace(labelpath, "JPEGImages", "labels", labelpath);
...
data load_data_detection(int n, char **paths, int m, int w, int h, int boxes, int classes, float jitter, float hue, float saturation, float exposure)
{
    ...
    for(i = 0; i < n; ++i){
        ...
        float dw = jitter * orig.w;  //为数据添加抖动干扰，提高网络的泛化能力(crop
...
data load_data_attack(char *path, int n, int w, int h, int num_box, int classes, box_label *boxes)
{
    data d = {0};
    d.shallow = 0;
    ...
    //对相应数据的标签进行读取图像坐标高宽id
    fill_truth_attack(num_box, d.y.vals[0], classes, boxes);
...
{
    pthread_t thread;
    struct load_args *ptr = calloc(1, sizeof(struct load_args));  //freed in load_thread()
    *ptr = args;
...
void *load_threads(void *ptr)
{
    ...
    *out = concat_datas(buffers, args.threads); //here, shallow of out is 1
    out->shallow = 0;
    for(i = 0; i < args.threads; ++i){
        buffers[i].shallow = 1;
        free_data(buffers[i]);
...
{
    int i;
    data out = {0};  //shallow is 0. when nothing in out, only need to free pointer **.
    for(i = 0; i < n; ++i){
        data new = concat_data(d[i], out);
        free_data(out);
        out = new;  //shallow is 1. something in out, need to free pointer * before **
    }
    return out;
}
...
void get_next_batch(data d, int n, int offset, float *X, float *y)  //y is net->truth, X is net->input
{
    int j;
    for(j = 0; j < n; ++j){
