/*********
network structure parser, parse the params of convolution layer
**********/
...
typedef struct{
    char *type;
    list *options;  //list struct at darknet.h, one section has needed params of a layer of CNN
}section;

list *read_cfg(char *filename);

LAYER_TYPE string_to_layer_type(char * type)  //return type of layer
{
...
void free_section(section *s)   //release memory of section structure, which is at the top of this file
{
    free(s->type);
...
typedef struct size_params{
    ...
    int time_steps;
    network *net;
} size_params;  //size_params indicate params of a layer

local_layer parse_local(list *options, size_params params)  //local_layer is defined in local_layer.h, it is structure layer anyway
{
...
convolutional_layer parse_convolutional(list *options, size_params params)  //parse convolutional layer, params is params of former layer
{
//get params
    int n = option_find_int(options, "filters",1);
    int size = option_find_int(options, "size",1);
    ...
    if(pad) padding = size/2;
//activate func are got by name matching
    char *activation_s = option_find_str(options, "activation", "logistic");
    ACTIVATION activation = get_activation(activation_s);  //enum, contain logistic relu,linear, leaky...
//get params of former layer
    int batch,h,w,c;
    h = params.h;
    w = params.w;
    c = params.c;
...
void parse_net_options(list *options, network *net)
{
    ...
    //add dirty and robust!!
    net->dirty = option_find_int_quiet(options, "dirty", 0);
    net->dirty_limit = option_find_int_quiet(options, "dirty_limit", 0);
    net->robust = option_find_int_quiet(options, "robust", 0);
    //end dirty!!
    net->output_bound = option_find_int_quiet(options, "output_bound", 0);
...
network *parse_network_cfg(char *filename)  //get filename as param, parse network framework
{
    list *sections = read_cfg(filename);  //get link of CFG file, make content in cfg file to a list
    node *n = sections->front;            //指向net块
...
/*parse golden output in right file
*used in image.c
*/
//id boxnum classnum objectness box1 x y w h prob1 prob2 ... prob80 objectness box2... prob80 id boxnum box1... \n
void save_rightline(int img_id, detection *dets, int nboxes, int classes, int max_box, FILE *rf)
{
    ...
    fwrite(&img_id, sizeof(int), 1, rf);                //img_id
    nboxes = (nboxes < max_box) ? nboxes : max_box;
    fwrite(&nboxes, sizeof(int), 1, rf);
    fwrite(&classes, sizeof(int), 1, rf);
    for(int i=0; i < nboxes; i++){
        fwrite(&dets[i].objectness, sizeof(float), 1, rf );
        box b = dets[i].bbox;
        fwrite(&b.x, sizeof(float), 1, rf);
        fwrite(&b.y, sizeof(float), 1, rf);
        fwrite(&b.w, sizeof(float), 1, rf);
        fwrite(&b.h, sizeof(float), 1, rf);
        fwrite(dets[i].prob, sizeof(float), classes, rf);
    }
    char end = '\n';
...
{
    int img_id, box_num, class_num;
    fread(&img_id, sizeof(int), 1, rf);
    //printf("%d\n", img_id);
    fread(&box_num, sizeof(int), 1, rf);
    //printf("%d\n", box_num);
    fread(&class_num, sizeof(int), 1, rf);
    //printf("%d\n", class_num);
    gold_ans *gd = create_network_boxes(img_id, box_num, class_num);
    for(int i=0; i < box_num; i++){
        fread(&gd[i].objectness, sizeof(float), 1, rf);
        fread(&gd[i].bbox.x, sizeof(float), 1, rf);
        fread(&gd[i].bbox.y, sizeof(float), 1, rf);
        fread(&gd[i].bbox.w, sizeof(float), 1, rf);
        fread(&gd[i].bbox.h, sizeof(float), 1, rf);
        //printf("%f %f %f %f %f\n", gd[i].objectness, gd[i].bbox.x, gd[i].bbox.y, gd[i].bbox.w, gd[i].bbox.h);
        fread(gd[i].prob, sizeof(float), class_num, rf);
        //for(int j=0; j < class_num; j++){
            //printf("%f ", gd[i].prob[j]);
        //}
        //printf("\n");
    }
