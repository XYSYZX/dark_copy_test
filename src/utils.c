...
float find_float_arg(int argc, char **argv, char *arg, float def)
{
    ...
    for(i = 0; i < argc-1; ++i){
        if(!argv[i]) continue;
        if(0==strcmp(argv[i], arg)){
            def = atof(argv[i+1]);  //jump the param in front of value, like '-prefix'
            del_arg(argc, argv, i);//delete the wanted value, in utils.c
            del_arg(argc, argv, i); //delete the space after wanted value
            break;
        }
    }
    return def;
...
char *basecfg(char *cfgfile)
{
    ...
    while((next = strchr(c, '/')))
    {
        c = next+1;  //detect where / show up, make c point to location after /
    }
    c = copy_string(c);   //in this file, copy c to c
    next = strchr(c, '.');
    if (next) *next = 0;
    return c;
}
...
char *itoa(char *a, int n)
{
	...
	for(;n>0;)
	{
		int temp=n%10;  //get geweishu
		b[count]=temp+'0';  //change to char
		n=n/10;
		count++;
	}
...
char *copy_to_gold(char *a, char *b)
{
	...
	while((*b)!=' '&&*b!='\0')
	{
		//printf("now b points at: %c*\n", *b);
		(*a)=(*b);
		a++;b++;
	}
	//printf("at the end of copy_to_gold, b points at: %c*\n", *b);
	return b;
}
...
{
    fwrite(&box, sizeof(int), 1, fp);
    fwrite(&a.x, sizeof(float), 1, fp);
    fwrite(&a.y, sizeof(float), 1, fp);
    fwrite(&a.w, sizeof(float), 1, fp);
    fwrite(&a.h, sizeof(float), 1, fp);
}
//a is right one
void print_prob(float *ap, FILE *fp, int classes)
{
    fwrite(ap, sizeof(float), classes, fp);
}
...
void print_img(double time, int img_num, int box_num, int classes, detection *dets, FILE *fp)
{
    ...
    fwrite(&img_num, sizeof(int), 1, fp);
    fwrite(&box_num, sizeof(int), 1, fp);
    fwrite(&classes, sizeof(int), 1, fp);

    for(int i = 0; i < box_num; i++){
        print_objectness(dets[i].objectness, fp);
        print_box_coord(dets[i].bbox, fp, i);
        print_prob(dets[i].prob, fp, classes);
    }
